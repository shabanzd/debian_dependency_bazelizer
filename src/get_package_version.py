...
def parse_debian_version(version_string: str) -> DebianVersion:
...
def compare_debian_versions(version1: str, version2: str) -> int:
    debian_version_1 = parse_debian_version(version1)
    debian_version_2 = parse_debian_version(version2)
...
def get_version_from_registry(
    ...
    version_spec = spec + parse_debian_version(version).version if version_spec else ""
    # find the highest version that matches the version_specifier
    for found_version in reversed(versions):   
        if satisfies_specification(parse_debian_version(found_version).version, version_spec):
            logger.debug("Found version: %s", parse_debian_version(found_version).version)
...
def get_package_version(registry_path: Path, name: str, arch: str, version_spec: str = "") -> str:
