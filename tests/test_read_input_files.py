...
def test_assertion_when_package_has_no_arch():
    ...
    with pytest.raises(
        ValueError,
        match="Entry 'iproute2' has unexpected format, expected 'name:arch=version' or 'name:arch'",
    ):
        read_input_files(
...
def test_correct_input_file():
    ...
    assert (
        PackageMetadata(name="iproute2", arch="amd64", version="1:1.2.4") in metadatas
    )
...
def test_arm_input_files():
    ...
    assert (
        str(e.value)
        == "Unsupported architecture arm64. Only amd64 arch is supported so far."
    )
